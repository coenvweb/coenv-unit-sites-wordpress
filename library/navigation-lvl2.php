...
function coenv_base_hierarchical_submenu($postid, $post_type = 'page') {
    ...
    return coenv_base_hierarchical_submenu_get_children($top_post, $post, $post_type);
}

function coenv_base_hierarchical_submenu_get_children($post, $current_page, $post_type) {
    $menu = '';
    ...
    if ($children) {
        ...
        foreach ($children as $child) {
            ...
            if (get_children($child->ID) && (in_array($child->ID, get_post_ancestors($current_page)) || ($child->ID == $current_page->ID))) {
                $menu .= coenv_base_hierarchical_submenu_get_children($child, $current_page, $post_type);
            }
            $menu .= "</li>\n";
        }
        $menu .= "</ul>\n";
