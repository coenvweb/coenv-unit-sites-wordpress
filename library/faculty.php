...
/* 
 * Return faculty links to research areas.
 */
function coenv_base_fac_terms_lite($id) {
	$fac_terms = wp_get_post_terms( $id, 'research_areas' );
	if ($fac_terms) {
		echo '<ul class="fac-terms inline-list">';
		foreach ($fac_terms as $term) {

			echo '<li><a class="button" href="/faculty-research/research-areas/' . $term->slug . '/">' . $term->name . '</a></li>';
		}
		echo '</ul>';
	}
}
