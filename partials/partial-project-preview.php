<?php
    $img = get_the_post_thumbnail_url(get_the_id(), 'large');
    $img_id = get_post_thumbnail_id(get_the_id());
    $meta = wp_get_attachment_metadata($img_id);
?>

<div class="project columns">
    <div class="project-wrap">
        <?php if (isset($img)) { ?> 
            <a href="<?php echo get_the_permalink();?>"><img class="project-preview-img large-4 columns left" src="<?=$img?>" alt="<?=isset($meta->title) ? $meta->title : ''?>" /></a>
        <?php }; ?>
        <?php if (isset($title)) { ?> 
        <div class="project-info columns large-8 right">
            <div class="post-content post-meta article__meta post-info columns">
                <span class="project-status"><?php the_field('project_status'); ?> |</span><span class="topics">
                <?php // Get categories
                    $terms = wp_get_post_terms(get_the_id(), 'project_topic');
                    $url_base = "/science/projects";
                    if (!empty($terms)) {
                        $terms_arr = array();

                        foreach ($terms as &$term) {
                            if ($term->slug != 'uncategorized') {
                                $terms_arr[] = '<a href="'.$url_base.'/?topic=' . $term->slug . '">' . $term->name . '</a>';
                            }
                        }
                        $terms_str = implode(', ', $terms_arr);

                    } else {
                        $terms_str = '';
                    }
                    $terms = "";
                ?>
                <?php if (!empty($terms_str)) {
                    echo $terms_str;
                } ?>
                </span>
            </div>
            <a href="<?php echo get_the_permalink();?>">
            <h2 class="project-preview-header"><?php echo get_the_title(); ?></h2>
            </a>
        </div>
        <?php }; ?>
    </div>
</div>
