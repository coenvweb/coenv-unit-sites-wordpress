...
jQuery( function ( $ ) {
	...
	$( '.ns-cloner-section-content select' ).not( '.no-chosen' ).each( function() {
		$(this).chosen({
			width: '100%',
	...
	$( document ).on(
		...
		function () {
			if ( window.confirm( "Are you sure you want to cancel this cloning process?" ) ) {
				...
				$( '.ns-cloner-processes-working' ).slideUp().after( '<img src="' + ns_cloner.loading_img + '" class="ns-cloner-loading-center" alt="loading..." />' );
			}
		}
	);
	...
	window.ns_cloner_form = {
		...
		'show_progress': function ( data ) {
			...
			for ( var process in data.processes ) {
				var progress = data.processes[ process ].progress;
				var label    = data.processes[ process ].label;
				...
				if ( item.length ) {
					...
					item.find( 'em:first' ).data( 'scroller' ).scrollTo( progress.completed );
					item.find( 'em:last' ).data( 'scroller' ).jumpTo( progress.total );
				} else {
