...
class OW_Revision_Service {
   ...
   public function api_check_is_compare_revision_allowed( $data ) {
      ...
      // capability check
      if ( ! current_user_can( 'ow_make_revision' ) || ! current_user_can( 'ow_make_revision_others' ) ) {         return new WP_Error( 'owf_rest_compare_revision', __( 'You are not allowed to check for revision.', 'oasisworkflow' ), array( 'status' => '403' )  );
   ...
   public function api_revision_compare( $data ) {
      ...
      if ( ! current_user_can( 'ow_make_revision' ) || ! current_user_can( 'ow_make_revision_others' ) ) {
         return new WP_Error( 'owf_rest_revision_compare', __( 'You are not allowed to compare revision', 'oasisworkflow' ), array( 'status' => '403' )  );
      }
   ...
   public function api_get_current_revision( $data ) {
      ...
      // capability check
      if ( ! current_user_can( 'ow_make_revision' ) || ! current_user_can( 'ow_make_revision_others' ) ) {
         return new WP_Error( 'owf_rest_get_current_revision', __( 'You are not allowed to create revision of the post.', 'oasisworkflow' ), array( 'status' => '403' )  );
      }
   ...
   public function api_is_make_revision_allowed( $data ) {
      ...
      // capability check
      if ( ! current_user_can( 'ow_make_revision' ) || ! current_user_can( 'ow_make_revision_others' ) ) {
          return new WP_Error( 'owf_rest_is_make_revision_allowed', __( 'You are not allowed to create revision of the post.', 'oasisworkflow' ), array( 'status' => '403' )  );
      }
      ...
      $show_button = false;
      if( $is_revise_action_available && in_array( $post_type, $allowed_post_types ) && $make_revision_enabled == "active" && $workflow_process_enabled == "active" ) {
         $show_button = true;
      }
   ...
   public function api_create_post_revision( $data ) {      
      ...
      if ( ! current_user_can( 'ow_make_revision' ) || ! current_user_can( 'ow_make_revision_others' ) ) {
         return new WP_Error( 'owf_rest_create_post_revision', __( 'You are not allowed to create revision of the post', 'oasisworkflow' ), array( 'status' => '403' )  );
      } 
...
//             	OW_Utility::instance()->logger( "adding to original");
   ...
   public function update_post_update_children( $original_post_id, $revision_post ) {
      ...
      // get children
      $children = get_posts( array( 'post_type' => 'any', 'numberposts' => -1, 'post_status' => 'any', 'post_parent' => $revision_post->ID ) );
   ...
   public function save_as_new_post( $status = '', $post_id = null ) {
      ...
      if ( ( ! ( isset( $_GET['post'] ) || isset( $_POST['post'] ) || ( isset( $_REQUEST['action'] ) && 'save_as_new_post_draft' == $_REQUEST['action'] ) ) ) && empty( $post_id ) ) {
