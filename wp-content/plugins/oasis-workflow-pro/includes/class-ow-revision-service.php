...
class OW_Revision_Service {
   ...
   public function update_published_post( $revised_post_id ) {
      ...
      $original_post_id = get_post_meta( $revised_post->ID, '_oasis_original', true );
    ...
    * @since 3.0
    *
    * Added delay to avoid any delete issues with the revision
    * @since 3.6
    *
    * @param $post_id
    * @param $post_status
    ...
    * @since 3.0
    */
   public function copy_post_meta_info( $new_id, $post ) {
    ...
    * @since 3.0
    */
   public function copy_post_copy_post_taxonomies( $new_id, $post ) {
      global $wpdb;
      if ( isset( $wpdb->terms ) ) {
    ...
    * @since 3.0
    */
   public function copy_post_copy_children( $new_id, $post ) {
      // get children
   ...
   public function update_post_meta_info( $original_post_id, $revised_post ) {
      ...
      foreach ( $post_meta_keys as $meta_key ) {
         $meta_key_trim = trim( $meta_key );
//          OW_Utility::instance()->logger( "the meta key is: " . $meta_key_trim);
         if ( '_edit_lock' == $meta_key_trim || //ignore keys like _edit_last, _edit_lock
         		'_edit_last' == $meta_key_trim ||
         		strpos( $meta_key_trim, 'oasis' ) !== false || // ignore any keys with 'oasis'
         		strpos( $meta_key_trim, 'ow_' ) !== false ) {// ignore any keys with ow_, like _oasis_task_priority
            continue;
         }
...
//             	OW_Utility::instance()->logger( "adding to original");
   ...
   public function create_copy( $post, $status = '', $parent_id = '' ) {
      ...
      if ( $post->post_type != 'attachment' ) {
         ...
         $status = 'draft';
         $new_post_author = OW_Utility::instance()->get_current_user()->ID;
      }
