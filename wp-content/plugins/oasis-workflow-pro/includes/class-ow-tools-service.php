...
//      is_plugin_active() is defined - wp-admin/includes/plugin.php - is only loaded in the admin, after your plugin is loaded
   ...
   */
   ...
   public function api_get_plugin_settings( $data ) {
      ...
      if ( ! current_user_can( 'ow_submit_to_workflow' ) && ! current_user_can( 'ow_sign_off_step' ) ) {
         ...
         return new WP_Error( 'owf_rest_cannot_submit', __( 'You are not allowed to fetch workflow settings.', 'oasisworkflow' ), array( 'status' => '403' )  );
      }
   ...
   public function api_check_is_active_editorial_comments( $data ) {
      ...
      if ( ! current_user_can( 'ow_submit_to_workflow' ) && ! current_user_can( 'ow_sign_off_step' ) ) {
         // Replaced wp_die to WP_Error to prevent 500 internal server error.
         return new WP_Error( 'owf_rest_editorial_comments', __( 'You are not allowed to view editorial comments.', 'oasisworkflow' ), array( 'status' => '403' )  );
      }
      ...
      if( is_plugin_active( 'ow-editorial-comments/ow-editorial-comments.php' ) || is_plugin_active_for_network( 'ow-editorial-comments/ow-editorial-comments.php' ) == 1 ) {
         return true;
      }
      return false;
   }
