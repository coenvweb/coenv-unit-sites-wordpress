...
class OW_Email_Settings_Helper
{
           ...
           <ul>
               <li>
                   <span class="description">
                      <?php echo __("Available template placeholders for email subject and content (Applicable to email recipients in the 'To' list only):", "oasisworkflow"); ?>
                   </span>
               </li>
              <?php
              if ($placeholders) {
   ...
   public function get_email_roles_option_list( $action, $selected_roles ) {
      ...
      $participating_roles['post_author'] = __( 'Post Author', 'oasisworkflow' );
      $participating_roles['post_submitter'] = __( 'Post Submitter', 'oasisworkflow' );
      ...
      if (( $action == OW_Email_Settings_Helper::UNAUTHORIZED_UPDATE_EMAIL ||
         ...
         $participating_roles['current_task_assignees'] = __( 'Current Task Assignees', 'oasisworkflow' );
      }
      ...
      asort($participating_roles);
		...
		foreach ( $participating_roles as $role => $name ) {
         if ( ! empty($selected_roles) && is_array($selected_roles) && in_array(esc_attr($role), $selected_roles)) { // preselect specified role
            $selected = " ' selected='selected' ";
         } else {
            $selected = '';
         }
   ...
   public function get_email_users_option_list( $selected_users ) {
      ...
      $options = '<optgroup label="' . __( 'Users', 'oasisworkflow' ) . '">';
		foreach ( $users as $user ) {
         if ( ! empty($selected_users) && is_array($selected_users) && in_array($user->ID, $selected_users)) { // preselect specified role
            $selected = " ' selected='selected' ";
         } else {
            $selected = '';
         }
   ...
   public function get_email_external_users_option_list( $selected_external_users ) {
      ...
      $options = '<optgroup label="' . __( 'External Users', 'oasisworkflow' ) . '">';
...
//            invalid entry, because either email address or first/last name is null, so ignore and continue
            ...
            if ( ! empty($selected_external_users) && is_array($selected_external_users) && in_array($key, $selected_external_users)) { // preselect specified role
               $selected = " ' selected='selected' ";
            } else {
               $selected = '';
            }
            $options .= "<option value='e@{$key}' $selected>".$values['fname']. " ". $values['lname'] ."</option>";
         }
      }
      $options .= '</optgroup>';
