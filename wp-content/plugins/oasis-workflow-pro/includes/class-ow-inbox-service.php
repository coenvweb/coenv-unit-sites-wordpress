...
class OW_Inbox_Service {
   ...
   public function get_reassign_users( $history_id ) {
	...
	public function get_table_header() {      
      ...
      $inbox_column_headers['last_signoff_by'] = "<th class='column-last-signoff-by'>" . __( "Last Signed off By", "oasisworkflow" ) . "</th>";
      
		$inbox_column_headers['workflow_name'] = "<th class='column-workflow-name'>" . __( "Workflow [Step]", "oasisworkflow" ) . "</th>";
      
		$inbox_column_headers['category'] = "<th class='column-category'>" . __( "Category", "oasisworkflow" ) . "</th>";
      ...
      $inbox_column_headers['comments'] = "<th class='column-comments'>" . __( "Comments", "oasisworkflow" ) . "</th>";
   ...
   public function get_table_rows( $inbox_data, $inbox_items, $inbox_column_headers, $unclaimed_task_count=0 ) {
         ...
         foreach ( $inbox_items as $inbox_item ) {
            ...
            if ( $count >= $start ) {
               ...
               if ( array_key_exists( 'title', $inbox_column_headers ) ) {
                  echo '<td class="post-title page-title column-title column-primary" data-colname="Post/Page"><strong>' . esc_html( $post->post_title );
                  ...
                  if ( $needs_to_be_claimed ) { // if the item needs to be claimed, only "Claim" action is visible
                     echo "<div class='row-actions'>
                                    <span>
                                       <a href='#' class='claim' id='claim' userid={$selected_user} actionid={$inbox_item->ID}>" . __( "Claim", "oasisworkflow" ) . "</a>
                                       <span class='loading'>$space</span>
                                    </span>
                                    |
                                    <span>
                                       <a href='#' id='claim-and-edit' class='claim-and-edit' userid={$selected_user} actionid={$inbox_item->ID}>" . __( "Claim and Edit", "oasisworkflow" ) . "</a>
                                       <span class='loading'>$space</span>
                                    </span>
                                 </div>";
                  } else {
               ...
               if ( array_key_exists( 'priority', $inbox_column_headers ) ) {
                  if ( get_option( 'oasiswf_priority_setting' ) == 'enable_priority' ) {
                     ...
                     echo "<td data-colname='Priority' class='column-priority' ><p class='post-priority $css_class-priority'>" . $priority_value . "</p></td>";
                  }
               }
               ...
               if ( array_key_exists( 'author', $inbox_column_headers ) ) {
                  echo "<td data-colname='Author' class='column-author'>" . OW_Utility::instance()->get_user_name( $user->ID ) . "</td>";
               }
               
               if ( array_key_exists( 'last_signoff_by', $inbox_column_headers ) ) {
                  echo "<td data-colname='Last Signed off By' class='column-last-signoff-by'>".$sign_off_by."</td>";
               }
               ...
               if ( array_key_exists( 'due_date', $inbox_column_headers ) ) {
                  ...
                  echo "<td data-colname='Due Date' class='column-due-date'><span class=' . $past_due_date_field_class . '>" . OW_Utility::instance()->format_date_for_display( $inbox_item->due_date ) . "</span></td>";
               }
      ...
      elseif ($unclaimed_task_count > 0):
         $else_unclaimed_task_message = sprintf( __( ' But there are currently <a href="admin.php?page=oasiswf-inbox&action=inbox-unclaimed&user=%s">%s unclaimed task(s)</a>.', 'oasisworkflow' ), $selected_user, $unclaimed_task_count );
         echo "<tr>";
   ...
   public function display_row_actions( $inbox_row_actions_data ) {
      ...
      if ( OW_Utility::instance()->is_post_editable_others( $post_id ) ) {
			$inbox_row_actions['edit'] =  "<a href='post.php?post={$post_id}&action=edit&oasiswf={$workflow_history_id}&user={$user_id}' class='edit' real={$post_id}>" . __( "Edit", "oasisworkflow" ) . "</a>";
		}

      if ( OW_Utility::instance()->is_post_editable_others( $post_id ) ) {
         $inbox_row_actions['view'] = "<a target='_blank' href='" . get_preview_post_link($post_id) . "'>" . __("View", "oasisworkflow") . "</a>";
      }
      
      if ( $hide_compare_button == "" && $original_post_id && OW_Utility::instance()->is_post_editable( $post_id ) ) {
			$inbox_row_actions['compare'] = "<a href='post.php?page=oasiswf-revision&revision={$post_id}&_nonce=" . wp_create_nonce( 'owf_compare_revision_nonce' ) . "' wfid='$workflow_history_id' class='compare-post-revision' postid='$post_id'>" . __( "Compare", "oasisworkflow" ) . "</a><span class='loading'>$space</span>";
		}
      
      if ( current_user_can( 'ow_sign_off_step' ) && OW_Utility::instance()->is_post_editable( $post_id ) ) {
         $inbox_row_actions['sign_off'] = "<a href='#' wfid='$workflow_history_id' postid='$post_id' class='quick_sign_off'>" . $sign_off_label . "</a><span class='loading'>$space</span>";
      }
      
      if ( current_user_can( 'ow_reassign_task' ) ) {
         $inbox_row_actions['reassign'] = "<a href='#' wfid='$workflow_history_id' class='reassign'>" . __( "Reassign", "oasisworkflow" ) . "</a><span class='loading'>$space</span>";
      }
      
      if ( current_user_can( 'ow_abort_workflow' ) ) {
         $inbox_row_actions['abort_workflow'] = "<a href='#' wfid='$workflow_history_id' postid='$post_id' class='abort_workflow'>" . $abort_workflow_label . "</a><span class='loading'>$space</span>";
      }
      ...
      if ( current_user_can( 'ow_view_workflow_history' ) ) {
         ...
         $inbox_row_actions['view_history'] = "<a href='$nonce_url'> " . __( "View History", "oasisworkflow" ) . "</a>";
      }
