...
class OW_Workflow_Export_Import {
	...
	public function multiple_versions_selected_notice() {
		$export_error = OW_Utility::instance ()->admin_notice ( array (
				'type' => 'update',
				'message' => 'Only one version of the workflow can be exported.'
		) );
		echo $export_error;
	}
	...
	public function no_workflow_selected_notice() {
		$export_error = OW_Utility::instance ()->admin_notice ( array (
				'type' => 'update',
				'message' => 'Please select at least one workflow to export.'
		) );
		echo $export_error;
	}
	...
	public function successful_import_notice() {
		$import_success = OW_Utility::instance ()->admin_notice ( array (
				'type' => 'update',
				'message' => 'Workflows were imported successfully.'
		) );
		echo $import_success;
	}
	...
	public function import_workflows() {
		...
		while ( ( $data = fgetcsv ( $handle, 0, "," ) ) !== FALSE ) {
			...
			$steps_data = unserialize ( $data [14] ); // all step info
			...
			for( $i = 0; $i < $count; $i++ ) {
				$steps = array (
					'step_info' => $steps_data [$i] ['step_info'],
					'process_info' => $steps_data [$i] ['process_info'],
					'workflow_id' => $workflow_id, // use the newly inserted workflow_id
					'create_datetime' => $steps_data [$i] ['create_datetime'],
					'update_datetime' => $steps_data [$i] ['update_datetime']
				);
				...
				// get the step name
				$step_name_temp = json_decode( $steps_data [$i] ['step_info'] )->step_name;
