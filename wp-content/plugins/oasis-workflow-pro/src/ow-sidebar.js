...
export class OasisWorkflowProComponent extends Component {
	...
	componentDidMount() {
		...
		wp.apiFetch({ path: '/oasis-workflow/v1/workflows/documentRevision/checkCompareCapability/postId=' + postId, method: 'GET' }).then(
			(revisiondata) => {
				this.setState({
		...
		wp.apiFetch({ path: '/oasis-workflow/v1/workflows/submit/checkRoleCapability/postId=' + this.props.postId + '/postType=' + this.props.postType, method: 'GET' }).then(
			(response) => {
				...
				if ( response.is_role_applicable && response.can_submit_to_workflow  ) {
					...
					if ( ( this.state.actionHistoryId || this.props.isCurrentPostPublished || this.props.isCurrentPostScheduled ) && ( showWorkflowSidebar === "show" ) ) {
