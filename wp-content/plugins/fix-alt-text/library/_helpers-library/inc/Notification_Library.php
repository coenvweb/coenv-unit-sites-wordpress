 ...
 */
abstract class Notification_Library extends Base_Library {
	...
	public static function schedule_cron(): void {
		...
		register_deactivation_hook( FIXALTTEXT_FILE, [
			static::class,
			'cron_deactivate',
		] );
	...
	public static function get_notifications( bool $force_update = false ): array {
		...
		if ( $force_update || empty( $fixalttext['notifications'] ) ) {
			$fixalttext['notifications'] = get_option( FIXALTTEXT_NOTIFICATIONS_OPTION );

			if ( ! is_array( $fixalttext['notifications'] ) ) {
				$fixalttext['notifications'] = [];
			}
		}

		return $fixalttext['notifications'];
	}
	...
	public static function save_notifications( array $notifications ): bool {
		...
		$result = update_option( FIXALTTEXT_NOTIFICATIONS_OPTION, $notifications, false );
	...
	public static function display_bell(): void {
		...
		echo '<a href="#notifications" id="notifications" class="dashicons dashicons-bell notifications-bell ' . esc_attr( $class ) . '"><span class="bell-inner"><span class="ada-hidden">' . esc_html__( 'Notifications', FIXALTTEXT_SLUG ) . '</span><span class="count">' . esc_html( $unread ) . '</span></span></a>
		<ul class="notifications" data-nonce="' . esc_attr( wp_create_nonce( static::get_nonce_key() ) ) . '" data-action="' . esc_attr( static::get_ajax_hook() ) . '"><li><h3>' . esc_html__( 'Notifications', FIXALTTEXT_SLUG ) . '</h3><a href="#all-read" class="mark-all-read">' . esc_html__( 'Mark All As Read', FIXALTTEXT_SLUG ) . '</a></li>';

		if ( empty( $notifications ) ) {
			echo '<li>' . esc_html__( 'No notifications found', FIXALTTEXT_SLUG ) . '</li>';
		} else {
			$count = 0;
			foreach ( $notifications as $notification ) {
				...
				echo ( $read_status ) ? '<input class="read-status" name="read_status" type="checkbox" CHECKED  aria-label="' . esc_attr__( 'Read Status', FIXALTTEXT_SLUG ) . '"/>' : '<input class="read-status" name="read_status" type="checkbox" aria-label="' . esc_attr__( 'Read Status', FIXALTTEXT_SLUG ) . '"/>';
	...
	public final static function update_read_status(): void {
		...
		if ( ! wp_verify_nonce( REQUEST::text_field( 'nonce' ), static::get_nonce_key() ) ) {
			...
			$response = [ 'html' => FIXALTTEXT_SLUG . ' - ' . __( 'Invalid nonce. Try refreshing the page to fix the issue.', FIXALTTEXT_SLUG ) ];
	...
	public static function get_ajax_hook(): string {
		...
		return FIXALTTEXT_HOOK_PREFIX . 'notification_read_update';
	...
	public static function get_cleanup_cron_hook(): string {
		...
		return FIXALTTEXT_HOOK_PREFIX . 'cleanup_notifications_cron';
