 ...
 * @since   1.0.0
 */
abstract class Menu_Library extends Base_Library {
	...
	public static function wp_delete_nav_menu( int $term_id ): void {
		...
		$cache_status_codes_option = Get::cache_status_codes_option_name('wp_delete_nav_menu', $term_id );

		Scan::reprocess_entries( $term_id, 'menu', $cache_status_codes_option );
	...
	public static function wp_update_nav_menu( int $menu_id ): void {
		...
		$cache_status_codes_option = Get::cache_status_codes_option_name('wp_update_nav_menu', $menu_id );

		$menu = static::get_by_id( $menu_id );
		if ( $menu ) {
			Scan::scan_menu( $menu, FIXALTTEXT_HELPERSLIBRARY_CURRENT_SITE_ID, $cache_status_codes_option );
		}
