 ...
 */
final class Settings_Display_Library {
	...
	public static function input( array $args ): void {
		...
		if ( ! empty( $attributes ) ) {
			foreach ( $attributes as $k => $v ) {
				$att_html .= $k . '="' . esc_attr( $v ) . '"';
			}
		}

		echo '<input type="' . esc_attr( $type ) . '" name="' . esc_attr( $property ) . '" class="' . esc_attr( $class ) . '" value="' . esc_attr( $value ) . '"' . esc_attr( $disabled ) . ' style="' . esc_attr( $style ) . '" ' . $att_html . ' />';
	...
	public static function checkboxes( array $args ): void {
		...
		foreach ( $options as $option_key => $option ) {
			...
			$specific_disabled = $option['disabled'] ?? false;
			$disabled_attr = ( $disabled || $specific_disabled ) ? ' DISABLED' : '';
			...
			echo '<div class="checkbox-row item-row"><input name="' . esc_attr( $property ) . '[]" value="' . esc_attr( $option['value'] ) . '" type="checkbox" id="' . esc_attr( $id ) . '" ' . esc_html( $checked ) . esc_html( $disabled_attr ) . ' /><label for="' . esc_attr( $id ) . '">' . esc_html( $option['label'] ) . '</label>';
		...
		foreach ( $rows['values'] as $row ) {
			...
			foreach ( $options as $option ) {
				...
				if ( isset( $row['excluded'] ) && in_array( $option['property'], $row['excluded'] ) ) {
					...
					$label = $rows['label'] . ' ' . $row['label'] . ' ' . __( 'has', FIXALTTEXT_SLUG ) . ' ' . $option['label'];
	...
	public static function list( string $heading, array $items, string $dashicon = '', bool $hr = false ): void {
		...
		echo '<ul>';
		if ( empty( $items ) ) {
			echo '<li>' . esc_html__( 'None selected', FIXALTTEXT_SLUG ) . '</li>';
		} else {
