...
abstract class Get_Library {
	...
	public final static function sites(): array {
		...
		if ( empty( static::$sites ) ) {
			...
			if ( is_multisite() ) {
				...
				$site = [];
				$site['blog_id'] = FIXALTTEXT_HELPERSLIBRARY_CURRENT_SITE_ID;
				$site['domain'] = str_replace( [
					'http://',
					'https://',
				], '', get_site_url() );

				$sites[FIXALTTEXT_HELPERSLIBRARY_CURRENT_SITE_ID] = (object) $site;
			}
	...
	public final static function current_site(): object {
		...
		return $sites[ FIXALTTEXT_HELPERSLIBRARY_CURRENT_SITE_ID ];
	}
	...
	public final static function current_site_property( string $property = 'domain' ): string {
		...
		return $sites[ FIXALTTEXT_HELPERSLIBRARY_CURRENT_SITE_ID ]->$property ?? '';
	}
	 ...
	 * @return string
	 */
	public static function cache_status_codes_option_name( string $action, int $post_id = 0, int $site_id = FIXALTTEXT_HELPERSLIBRARY_CURRENT_SITE_ID ): string {

		$key_name = FIXALTTEXT_HOOK_PREFIX . $action . '_' . $site_id;
	...
	public final static function subheader(): string {
		...
		if ( is_multisite() ) {
			if ( ! is_network_admin() ) {
				$subheading = '<h2 style="text-align:center">' . esc_html( static::current_site_property() ) . '</h2>';
			}
		}
	...
	public static function excluded_post_types(): array {
		...
		return apply_filters( FIXALTTEXT_HOOK_PREFIX . 'excluded_post_types', $excluded_post_types );
	}
	...
	public static function excluded_taxonomies(): array {
		...
		return apply_filters( FIXALTTEXT_HOOK_PREFIX . 'excluded_taxonomies', $excluded_taxonomies );
	}
	...
	public static function using_network_media_library() {
		...
		if ( ! defined( 'FIXALTTEXT_HELPERSLIBRARY_NETWORK_MEDIA_LIBRARY' ) ) {
			$active = is_multisite() && class_exists( '\Network_Media_Library\ACF_Value_Filter' );
			define( 'FIXALTTEXT_HELPERSLIBRARY_NETWORK_MEDIA_LIBRARY', $active );
		}

		return FIXALTTEXT_HELPERSLIBRARY_NETWORK_MEDIA_LIBRARY;
